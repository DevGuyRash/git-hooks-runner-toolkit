- [x] 1. Build CLI matrix runner for command coverage
...
- [ ] 2. Add lifecycle Bats suite consuming matrix output
  - Files: tests/audit/lifecycle.bats; tests/ephemeral/lifecycle.bats (update)
  - Assert hooks-path, manifest restoration, overlay ordering, and truncation regressions using matrix cases; extend existing ephemeral lifecycle tests to reference matrix data.
