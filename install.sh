...
print_update_usage() {
  cat <<'HELP'
NAME
    githooks update - Refresh managed runner assets and staged hook parts.

SYNOPSIS
    githooks update [OPTIONS]

DESCRIPTION
    Reinstalls the toolkit files already present in the repository without
    altering Git configuration. The command rewrites managed stub dispatchers,
    copies the latest shared libraries, and updates any staged hook parts whose
...
OPTIONS
    --mode MODE
        Target a specific installation mode. Use `ephemeral` to refresh the
        manifest-managed installation under `.git/.githooks/`.

    -n, --dry-run
        Describe the actions that would be taken without modifying files.
...
NOTES
    The update process preserves user configuration and only touches files
    previously installed by the toolkit. Staged scripts that lack a matching
    source remain unchanged.

EXAMPLES
    githooks update
        Refresh a standard installation and restage matching example scripts.

    githooks update --mode ephemeral
        Refresh the Ephemeral Mode installation recorded in `.git/.githooks/`.
HELP
}
...
update_detect_managed_hooks() {
  UPDATE_MANAGED_HOOKS=""
  update_hooks_root=$(githooks_hooks_root)
  if [ -d "${update_hooks_root}" ]; then
    update_stub_list=$(LC_ALL=C find "${update_hooks_root}" -mindepth 1 -maxdepth 1 -type f -perm -u+x -print 2>/dev/null | LC_ALL=C sort)
    if [ -n "${update_stub_list}" ]; then
      update_newline=$(printf '\n_')
      update_saved_ifs=$IFS
      IFS=${update_newline%_}
      set -f
      for update_stub_path in ${update_stub_list}; do
        if grep -q 'generated by .githooks-runner' "${update_stub_path}" 2>/dev/null; then
        stage_append_unique UPDATE_MANAGED_HOOKS "$(basename "${update_stub_path}")"
        fi
      done
      set +f
      IFS=${update_saved_ifs}
    fi
  fi
  printf '%s\n' "${UPDATE_MANAGED_HOOKS}"
}
...
update_refresh_stub() {
  ...
  _update_hook=$1
  if [ -z "${_update_hook}" ]; then
    return 0
  fi
  update_force_saved=${FORCE}
  FORCE=1
  if [ "${DRY_RUN}" -eq 1 ]; then
    githooks_log_info "DRY-RUN: refresh stub ${_update_hook}"
  fi
  write_stub "${_update_hook}"
  FORCE=${update_force_saved}
  UPDATE_REFRESHED_STUBS=$((UPDATE_REFRESHED_STUBS + 1))
}

update_find_source_for_part() {
  if [ "$#" -ne 1 ]; then
    return 1
  fi
  _update_name=$1
  if [ -z "${_update_name}" ]; then
    return 1
  fi
  for _update_base in "${SCRIPT_DIR%/}/hooks" "${SCRIPT_DIR%/}/examples"; do
    if [ ! -d "${_update_base}" ]; then
      continue
    fi
    _update_match=$(LC_ALL=C find "${_update_base}" -type f -perm -u+x -name "${_update_name}" -print 2>/dev/null | LC_ALL=C sort | head -n 1)
    if [ -n "${_update_match}" ]; then
      printf '%s\n' "${_update_match}"
      return 0
    fi
  done
...
update_replace_file() {
  if [ "$#" -lt 3 ]; then
    githooks_die "update_replace_file expects source, destination, and mode"
  fi
  _update_src=$1
  _update_dest=$2
  _update_mode=$3
  _update_label=${4-$(basename "${_update_dest}")}
  if [ ! -f "${_update_src}" ]; then
    githooks_log_warn "update source missing: ${_update_src}"
    return 1
  fi
  _update_dir=$(dirname "${_update_dest}")
  if [ ! -d "${_update_dir}" ]; then
    githooks_mkdir_p "${_update_dir}"
  fi
  if [ "${DRY_RUN}" -eq 1 ]; then
    githooks_log_info "DRY-RUN: update ${_update_label}"
    return 0
  fi
  _update_tmp="${_update_dest}.update.$$"
  if ! cp "${_update_src}" "${_update_tmp}"; then
    rm -f "${_update_tmp}" 2>/dev/null || true
    githooks_die "Failed to update ${_update_dest}"
  fi
  githooks_chmod "${_update_mode}" "${_update_tmp}"
  if ! mv "${_update_tmp}" "${_update_dest}"; then
    rm -f "${_update_tmp}" 2>/dev/null || true
    githooks_die "Failed to replace ${_update_dest}"
  fi
  githooks_log_info "update ${_update_label}"
  return 0
}

update_refresh_parts_for_root() {
  if [ "$#" -ne 1 ]; then
    return 0
  fi
  _update_parts_root=$1
  if [ -z "${_update_parts_root}" ]; then
    return 0
  fi
  if ! githooks_require_readable_parts "${_update_parts_root}"; then
    return 0
  fi
  _update_dirs=$(LC_ALL=C find "${_update_parts_root}" -mindepth 1 -maxdepth 1 -type d -name '*.d' -print 2>/dev/null | LC_ALL=C sort)
  if [ -z "${_update_dirs}" ]; then
    return 0
  fi
  _update_newline=$(printf '\n_')
  _update_saved_ifs=$IFS
  IFS=${_update_newline%_}
  set -f
  for _update_dir in ${_update_dirs}; do
    _update_parts=$(LC_ALL=C find "${_update_dir}" -mindepth 1 -maxdepth 1 -type f -name '*.sh' -perm -u+x -print 2>/dev/null | LC_ALL=C sort)
    if [ -z "${_update_parts}" ]; then
      continue
    fi
    for _update_part in ${_update_parts}; do
      _update_name=$(basename "${_update_part}")
      _update_source=$(update_find_source_for_part "${_update_name}" || true)
      if [ -z "${_update_source}" ]; then
        UPDATE_PARTS_SOURCE_MISSING=$((UPDATE_PARTS_SOURCE_MISSING + 1))
        githooks_log_info "update skip ${_update_name}: source not found"
        continue
      fi
      if [ "${FORCE}" -eq 0 ] && stage_files_identical "${_update_source}" "${_update_part}"; then
        UPDATE_PARTS_SKIPPED_IDENTICAL=$((UPDATE_PARTS_SKIPPED_IDENTICAL + 1))
        continue
      fi
      _update_label=${_update_part#${_update_parts_root%/}/}
      update_replace_file "${_update_source}" "${_update_part}" 755 "${_update_label}"
      UPDATE_PARTS_REFRESHED=$((UPDATE_PARTS_REFRESHED + 1))
    done
  done
  set +f
  IFS=${_update_saved_ifs}
}
...
update_count_hooks() {
  ...
  _update_hook_list=$1
  if [ -z "${_update_hook_list}" ]; then
    printf '0\n'
    return 0
  fi
  _update_hook_list=$(printf '%s' "${_update_hook_list}" | tr '\n' ' ')
  _update_saved_ifs=$IFS
  IFS=' '
  set -f
  set -- ${_update_hook_list}
  set +f
  IFS=${_update_saved_ifs}
  printf '%d\n' "$#"
}
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --)
        ...
        ;;
      -* )
        githooks_die "unknown option for update --mode ephemeral: $1"
        ;;
      *)
        githooks_die "unexpected argument for update --mode ephemeral: $1"
        ;;
    esac
  done

  if [ "$#" -gt 0 ]; then
    githooks_die "update --mode ephemeral does not accept positional arguments"
  fi

  update_reset_counters

  EPHEMERAL_ROOT_DIR=$(ephemeral_root_dir)
  EPHEMERAL_MANIFEST_PATH=$(ephemeral_manifest_path)
  if [ ! -d "${EPHEMERAL_ROOT_DIR}" ] && [ ! -f "${EPHEMERAL_MANIFEST_PATH}" ]; then
    githooks_log_info "Ephemeral Mode not installed; nothing to update"
    return 0
  fi

  EPHEMERAL_MANAGED_HOOKS=$(ephemeral_manifest_get MANAGED_HOOKS || true)
  if [ -n "${EPHEMERAL_MANAGED_HOOKS}" ]; then
    # shellcheck disable=SC2086
    ephemeral_refresh ${EPHEMERAL_MANAGED_HOOKS}
  else
    ephemeral_refresh
  fi

  UPDATE_REFRESHED_STUBS=$(update_count_hooks "$(ephemeral_manifest_get MANAGED_HOOKS || true)")

  shared_root=$(githooks_shared_root)
  hooks_root=$(githooks_hooks_root)

  EPHEMERAL_ROOT_LIST=$(ephemeral_manifest_roots)
  if [ -n "${EPHEMERAL_ROOT_LIST}" ]; then
    EPHEMERAL_ROOT_SERIAL=$(printf '%s\n' "${EPHEMERAL_ROOT_LIST}" | tr ':' '\n')
    while IFS= read -r EPHEMERAL_ROOT_ITEM || [ -n "${EPHEMERAL_ROOT_ITEM}" ]; do
      if [ -n "${EPHEMERAL_ROOT_ITEM}" ] && [ -d "${EPHEMERAL_ROOT_ITEM}" ]; then
        update_refresh_parts_for_root "${EPHEMERAL_ROOT_ITEM}"
      fi
    done <<EOF
...
cmd_update() {
  ...
  update_repo_git_dir=$(githooks_repo_git_dir)
  update_is_ephemeral=0
  if [ "${update_current_shared_root}" = "${update_repo_git_dir%/}/.githooks/parts" ]; then
    update_is_ephemeral=1
  fi
  if [ "${UPDATE_RESOLVED_MODE}" = "ephemeral" ]; then
    cmd_update_ephemeral "$@"
    return 0
  fi

  if [ "${update_is_ephemeral}" -eq 1 ]; then
    cmd_update_ephemeral "$@"
    return 0
  fi
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --)
        ...
        ;;
      -* )
        githooks_die "unknown option for update: $1"
        ;;
      *)
        githooks_die "unexpected argument for update: $1"
        ;;
    esac
  done
  ...
  shared_root=${update_current_shared_root}
  hooks_root=${update_current_hooks_root}
  runner_target="${shared_root%/}/_runner.sh"
  lib_target="${shared_root%/}/lib/common.sh"
  watch_lib_target="${shared_root%/}/lib/watch-configured-actions.sh"
  shared_root_is_ephemeral=${update_is_ephemeral}

  update_reset_counters

  install_runner

  managed_hooks=$(update_detect_managed_hooks)
  if [ -n "${managed_hooks}" ]; then
    for update_stub in ${managed_hooks}; do
      update_refresh_stub "${update_stub}"
    done
  else
    githooks_log_info "no managed stubs detected; skipping stub refresh"
  fi
